...
export default function App() {
    ...
    async function getTarget(habitName) {
        const response = await fetch(`${IP}/getTarget?habitName=${encodeURIComponent(habitName)}`)
    ...
    async function getRoutine(habitName) {
        const response = await fetch(`${IP}/getRoutine?habitName=${encodeURIComponent(habitName)}`)
        const data = await response.json();
        setRoutine(data);
        console.log(data);
        return data;
    ...
    return (
        <View style={styles.container}>
            ...
            {Object.entries(habitData).map(([key, value]) => (
                <Text style={styles.textColor} key={key}>{`${key}: ${value} `}</Text>
            ))}

            <Button title="New habit row" onPress={() => newHabitRow(habitName)} />
