...
export function getMockHistory() {
    ...
    const todaysday = (new Date().getDay() + 6) % 7; // Shift Sunday to the end
    // For each remaining day in todaysday, set historyCounts[day] to a random int
    for (let day = todaysday; day >= 0; day--) {
        historyCounts[day] = Math.floor(Math.random() * 100); // Random int between 0 and 99
    }
...
export async function calculateStreak(habitHistory) {
    ...
    if (habitHistory[Object.keys(habitHistory)[Object.keys(habitHistory).length - 1]].count > criteria) {
        streak++;
    }
